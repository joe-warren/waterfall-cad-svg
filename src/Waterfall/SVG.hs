...
import Linear (Epsilon, V2 (..), zero, nearZero)
...
closeLoop' :: (Waterfall.AnyPath point path, Monoid path, Num point, Epsilon point) => path -> path
closeLoop' p = 
    let (s, e) = Waterfall.pathEndpoints p
     in if nearZero (s - e) 
            then p
            else p <> Waterfall.line e s
                ...
                Svg.EndPath -> 
                    if null segments 
                        then go rest (o, []) paths
                              ...
                              in go rest (o, []) (closeLoop' newPath : paths)
                other -> Left (SVGPathError $ "Unsupported Path Command: " <> show other)
